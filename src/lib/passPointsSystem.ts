...
export const USER_TIERS: UserTier[] = [
  ...
  },
  {
    ...
    icon: '⭐',
    benefits: ['VIP matching', 'Mentor status', 'Premium features', 'Exclusive NFTs']
  },
  {
    name: 'Master',
    minPoints: 10000,
    ...
    icon: '👑',
    benefits: ['Ultimate priority', 'Community leader', 'All features unlocked', 'Legendary NFTs']
  }
];
...
export const getUserTier = (passPoints: number): UserTier => {
  return USER_TIERS.find(tier => 
    passPoints >= tier.minPoints && passPoints <= tier.maxPoints
  ) || USER_TIERS[0];
};
...
export const getPassPointsForNextLevel = (currentPassPoints: number): number => {
  const currentLevel = calculateLevel(currentPassPoints);
  return currentLevel * 1000;
};

// Calculate Pass Points needed for next tier
export const getPassPointsForNextTier = (currentPassPoints: number): { 
  ...
  const nextTier = currentTierIndex < USER_TIERS.length - 1 
...
export const awardPassPoints = async (
  ...
  try {
    ...
    console.log(`✅ Awarded ${reward.amount} Pass Points to ${walletAddress} for ${reward.action}`);
    return true;
...
export const updateSessionStats = async (
  ...
  try {
    ...
    if (sessionsCompleted % 5 === 0 && sessionsCompleted <= 10) {
      await awardPassPoints(walletAddress, sessionsCompleted === 5 ? 'STREAK_5' : 'STREAK_10');
    }
